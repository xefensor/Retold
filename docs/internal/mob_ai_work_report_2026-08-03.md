...
| Exact flight-versus-feeding command | `retold:fleeing_sheep_cannot_eat_until_danger_ends` passed 1/1 unthrottled on 2026-08-05. It starts a real vanilla Sheep graze, applies `FLEE`, confirms the delayed vanilla block transaction plus ordinary Retold forage and dropped-Wheat transactions all remain blocked without hunger/source changes, then confirms exactly one Wheat is eaten after flight clears. No wildcard, full GameTest, or TPS matrix was selected: the changed contract is one constant-time ownership check shared at meal completion, and the exact regression exercises both vanilla and Retold Sheep paths. |
...
The 2026-08-05 flight/feeding correction now makes vanilla block grazing and every Retold meal
transaction recheck urgent ownership at completion time. The exact unthrottled Sheep regression
passes 1/1, covering interrupted vanilla grazing, Retold forage, dropped-food conservation, and
normal one-item feeding after `FLEE` ends; the developer's reported Wolf scenario still needs an
in-game recheck.
