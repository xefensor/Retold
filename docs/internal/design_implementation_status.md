...
| Aender is sky-like/floating islands | Implemented / needs verification | `AenderChunkGenerator`, `AenderIslandSampler`, Aender biome/data/assets. Fresh worlds select generator V2 with order-independent overlap composition and cross-chunk large decorations; upgraded saves remain on legacy V1. Living Aender grass/leaves and the complete renewable Aender wood family have vanilla-style survival behavior, tags, loot, recipes, stripping, composting, signs, boats, and placement in the equivalent vanilla creative tabs; deterministic GameTests cover the server-side integration. The wood textures are explicitly AI-generated placeholders. V2 and the new client models still need multi-seed visual and chunk-border verification in the real dimension. |
| Dimension of change / chunks regenerate differently | Implemented / needs verification | `AenderRealityData` persists the seed/global/regional epochs/generator version, while the `retold:aender_chunk_reality` attachment persists each chunk's current or stale generation identity. Saved placed/broken blocks therefore survive runtime-cache loss, quit, and restart until their region legitimately changes; unloading evicts only the duplicate runtime signature, which is restored from the attachment on demand. In multiplayer, 384-block generator regions independently advance after their final client watcher leaves; a dependency halo prevents cross-region island seams. Stale loaded chunks are blanked only while unwatched, their non-player entities are discarded before the temporary empty interval, stabilized chunks are preserved, and rebuilding expands from approaching players. Tests cover serialization, attachment state transitions and cache restoration, entity cleanup, watcher lifecycle, and stable-versus-volatile policy. A three-start dedicated-server check on an isolated affected-world copy verified place/save/reload and break/save/reload; rapid multiplayer re-entry and crash recovery still need real-dimension verification. |
...
| Portal charge prepares destination terrain | Implemented / needs performance verification | Survival/adventure charge for at least 100 ticks; all players entering the Aender remain in the portal beyond their configured minimum when necessary until the 5x5 destination core is loaded and current. `AenderPortalWarmup` uses a portal ticket plus an average-tick-time-aware budget capped at 12 ms/32 chunks per tick and continues preparing the wider view opportunistically without technical action-bar text. Normal portal travel performs no synchronous Aender chunk generation; a bounded synchronous fallback remains only for commands, other integrations, and non-player entities that bypass the warm-up. |
