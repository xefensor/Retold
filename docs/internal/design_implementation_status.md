...
| Dimension of change / chunks regenerate differently | Implemented / needs verification | `AenderVolatility`, chunk load/world tick systems, generation signatures, synchronous arrival regeneration, stabilizer behavior, and section-level stale-chunk replacement. GameTests cover stable-versus-volatile regeneration policy; rapid re-entry in the real Aender and multiplayer still need in-game verification. |
