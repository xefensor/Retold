...
| Aender is sky-like/floating islands | Implemented / needs verification | `AenderChunkGenerator`, `AenderIslandSampler`, `AenderIslandArchetype`, `AenderDecorationPlanner`, `AenderBiomeSource`, Aender biome/data/assets. Fresh worlds select generator V3: V2's order-independent overlap composition and chunk-halo decoration plus deterministic per-island 3D biome assignment. Plains and the experimental inverted Aender Desert now own separate archetype families, surface/underside profiles, vertical distribution, erosion, warped detached satellites, decoration patches, and rare spire/crater formations. Rock arches and bridges were intentionally removed; satellite fragments, split ravines, and erosion openings remain separated. Stacked islands can resolve to different biomes and volatile regeneration replaces biome containers together with terrain. Upgraded saves retain their persisted V1/V2 generator selection. Pure planners have deterministic coverage, while V3 palettes, every silhouette/profile/formation, vertical biome transitions, client models, performance, and chunk borders still need multi-seed real-dimension visual verification. |
