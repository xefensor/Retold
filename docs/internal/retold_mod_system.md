...
- `AenderBiomeSource`
- `AenderIslandSampler`
- `AenderIslandArchetype`
- `AenderUndersideProfile`
- `AenderDecorationPlanner`
...
- `data/retold/worldgen/biome/aender_desert.json`
...
- `AenderPortalWarmup` refreshes a vanilla portal ticket and incrementally prepares loaded stale chunks using an adaptive budget derived from average server tick time, capped at 12 ms/32 chunks per tick. It spends only half of the headroom below 90% of the configured tick duration and pauses when that reserve is exhausted. A visibility pass first requires every already-loaded destination-view chunk to be current, stable, or blank; only then does the 5x5 regenerated arrival core complete the travel gate. This hides deletion without forcing the whole view to regenerate and uses no technical action-bar text. The wider view continues opportunistically. Commands, non-player entities, and third-party integrations that bypass the warm-up retain a bounded synchronous 5x5 fallback.
...
- Fresh worlds select generator V3. It retains V2's merged vertical island intervals and chunk-halo decorations, then assigns a deterministic biome to each island in three dimensions.
- Worlds upgraded from a release without `AenderRealityData` default to legacy generator V1 for all future Aender chunks.
- `AenderBiomeSource` resolves biome cells using X/Y/Z and the same deterministic island plan as terrain. Vertically stacked islands can therefore have different biomes without chunk-aligned biome borders. Void columns fall back to Aender Plains. Its thread-local one-chunk query cache samples each horizontal island column once and reuses it through the vertical biome cells; a volatility revision invalidates the cache after global or regional reality changes.
- Generator V3 currently gives each island either Aender Plains or the experimental inverted Aender Desert. Plains choose between round, elongated, crescent, split, and twin silhouettes; deserts choose plateau, dune-field, eroded-mesa, and thin-shelf silhouettes. Regional vertical drift breaks the fixed layer rhythm, while some region columns reuse a horizontally jittered anchor to create intentional stacked clusters. A parent island may also own up to three detached satellite fragments. Satellite fragments use independently stretched axes, multi-scale coast warp, strong angular lobes, rough surface relief, and optional shoreline bites rather than clean ellipses.
- Biome and archetype select the surface profile: plains use subdued broad hills and shallow valleys, while desert islands are 12% wider, 28% thinner, and use low relief, shallow basins, optional dune waves or mesa steps, and a sharper rim transition. Deterministic openings cut entirely through selected footprints. Tapered, rooted, fractured, and terraced underside profiles vary body depth independently; rooted plains increase hanging vegetation, while desert undersides expose sandstone spurs. Desert islands use deep denim Aender sand over medium periwinkle Aender sandstone, retain Aender stone cores, grow lavender Aender cacti, omit plains trees/lakes, and expose a softer rose-lavender sky/fog palette with subtle warped-spore energy particles. The palette is Aender-styled rather than a literal mathematical inversion of the Overworld desert.
- `AenderDecorationPlanner` keeps feature placement pure and coordinate-deterministic. Grass, flowers, trees, and cacti form separate patches with intentional clearings instead of uniform scatter. A sparse area-intersection pass reserves and places cross-chunk stone/sandstone spires and shallow energy craters without expanding the per-column decoration halo. The generator deliberately creates no rock arches or bridges; satellite fragments, split ravines, and erosion openings remain separated. Crater carving preserves embedded portal-frame deposits and refuses to cut a surface owned by an overlapping island.
- `AenderCavePlanner` creates deterministic cave-network nodes before deposits, decorations, and ore, while `AenderCaveCarver` owns their bounded chunk-local application. Caves are limited to substantial round/elongated plains and plateau/dune desert island bodies; fragile or already-open silhouettes are excluded. Eligible islands at least 92 blocks on their long radius and 30 blocks thick always receive one valid main passage after bounded alternate-path attempts, often receive a second, and exceptionally broad islands can receive a third. Smaller eligible islands use biome-specific cave chances. Main passages use two asymmetric cubic control turns plus a smaller local meander so their endpoints remain stable while the interior changes direction. A bounded deterministic pass gives 72% of valid main passages one connected interior branch; exceptionally broad islands can add a second. Branches depart at 41–83 degrees, curve independently, stay enclosed by conservative terrain shells, and can widen into a terminal chamber. Each main passage independently selects restrained surface, side, or underside openings for its start and end, allowing enclosed, single-ended, and double-ended routes. Carving aborts paths interrupted by existing erosion and preserves terrain simultaneously owned by an overlapping island.
- `AenderOrePlanner` deterministically places diamond-like Aenderite deposits across chunk borders: mostly 3–4 and 4–8 block veins with rare 8–12 block veins. Candidate origins are biased into the lower 55% of an island column, and the generator replaces only Aender Stone after terrain formations and portal-frame deposits are complete.
- Loaded volatile regeneration refills a stale chunk's biome containers from the current island plan before rebuilding its blocks. Stabilized chunks preserve both their saved terrain and biome.
- Decoration includes clustered grass/flowers/cacti, tree groves, lakes, biome/profile-aware underside spurs and growth, rare terrain formations, the Aender block palette, and provisional portal-frame deposits.
...
- Aenderite Ore requires a Netherite Pickaxe: it participates in `needs_diamond_tool` for lower tiers and explicitly denies Diamond-tool drops while remaining valid for Netherite. Silk Touch preserves the ore; otherwise Fortune applies to Raw Aenderite, which smelts or blasts into an Aenderite Ingot. The ingot intentionally has no crafting use yet.
...
- Procedural island bounds include coast warp/lobes and satellite fragments so islands are not clipped at chunk boundaries. Trees, boulders, spires, and craters evaluate origins in a surrounding halo and write only the current chunk's portion.
...
- Normal portal arrival uses a ticket to load the destination view, blanks stale loaded view chunks before travel, then incrementally replaces the 5x5 core, rebuilds heightmaps/light section state, reconciles entities, and resends changed chunks before allowing travel. Stale load callbacks blank and enqueue chunks instead of rebuilding them synchronously, and portal lookup skips stale chunks rather than regenerating every inspected candidate. Wider-view regeneration remains bounded and asynchronous. Direct teleports and non-player portal use retain the bounded synchronous core fallback because they do not have a charging phase.
